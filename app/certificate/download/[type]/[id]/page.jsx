...
export default function CertificateDownloadByTypePage() {
  ...
  const fetchCertificate = async () => {
    try {
      // Try different roll number formats
      const rollNumberVariants = [
        `${type}/${id}`,  // Dt/1
        `${type}-${id}`, // Dt-1
        id,              // Just the ID
        `${type}${id}`,  // Dt1
      ...
      for (const rollNumber of rollNumberVariants) {
        try {
          const response = await fetch(`/api/certificate/${encodeURIComponent(rollNumber)}`);
          if (response.ok) {
            const data = await response.json();
            if (data.certificate) {
              certificateFound = data.certificate;
              break;
            }
          }
        } catch (err) {
          console.error(`Error trying roll number ${rollNumber}:`, err);
        }
      }

      if (certificateFound) {
        setCertificate(certificateFound);
      } else {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-4">No certificate found for: {type}/{id}</p>
          <a
            href="/certificate"
