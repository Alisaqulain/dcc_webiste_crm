      ...
      try {
        ...
        if (c) {
          ...
          if (Array.isArray(c.heroSlides)) {
            ...
            console.log('Slide images:', c.heroSlides.map(s => ({ id: s.id, image: s.image })));
            setSlides(c.heroSlides);
          } else {
            console.warn('heroSlides is not an array:', typeof c.heroSlides, c.heroSlides);
          }
          
          if (Array.isArray(c.packages)) {
            console.log('Loading packages from database:', c.packages.length);
            console.log('Package images:', c.packages.map(p => ({ title: p.title, image: p.image })));
            setPackages(c.packages);
          } else {
            console.warn('packages is not an array:', typeof c.packages, c.packages);
          }
          
          if (Array.isArray(c.instructors)) {
            console.log('Loading instructors from database:', c.instructors.length);
            console.log('Instructor images:', c.instructors.map(i => ({ name: i.name, img: i.img })));
            setInstructors(c.instructors);
          } else {
            console.warn('instructors is not an array:', typeof c.instructors, c.instructors);
          }
          
          if (Array.isArray(c.testimonials)) {
            console.log('Loading testimonials from database:', c.testimonials.length);
            console.log('Testimonial images:', c.testimonials.map(t => ({ author: t.author, image: t.image })));
            setTestimonials(c.testimonials);
          } else {
            console.warn('testimonials is not an array:', typeof c.testimonials, c.testimonials);
          }
        } else {
          console.error('No content found in database response. API returned:', data);
        }
      } catch (e) {
            ...
            {instructors.map((ins, idx) => (
              ...
              >
                <div
                  ...
                  style={{
                    boxShadow: "0 20px 40px rgba(0,0,0,0.3), 0 0 0 1px rgba(255,255,255,0.1), inset 0 1px 0 rgba(255,255,255,0.2), inset 0 -1px 0 rgba(0,0,0,0.3)"
                  }}
                >
