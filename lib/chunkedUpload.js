...
export class ChunkedUploader {
  constructor(file, options = {}) {
    ...
    this.uploadUrl = options.uploadUrl || '/api/admin/video-upload-chunk';
  }
  ...
  async upload() {
    try {
      ...
      for (let i = 0; i < totalChunks; i++) {
        ...
        // Retry logic for each chunk with timeout
        let response;
        let lastError;
        ...
        for (let retry = 0; retry <= maxChunkRetries; retry++) {
          try {
            // Create AbortController for timeout (2 minutes per chunk)
            ...
            try {
              ...
              if (fetchError.name === 'AbortError') {
                throw new Error('Chunk upload timeout');
              }
              throw fetchError;
            }
          } catch (error) {
            ...
            if (retry === maxChunkRetries || !isNetworkError) {
              throw error;
            }
            
            // Exponential backoff: 1s, 2s, 4s
            const delay = 1000 * Math.pow(2, retry);
            console.log(`Chunk ${i + 1} upload failed, retrying in ${delay}ms... (attempt ${retry + 1}/${maxChunkRetries + 1})`);
