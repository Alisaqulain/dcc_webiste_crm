...
export class StorageService {
  constructor() {
    ...
    // Force filesystem mode on KVM hosting
    // Check if we can access filesystem (existsSync check)
    try {
      const { existsSync } = require('fs');
      this.isServerless = false; // Force filesystem on KVM
    } catch (e) {
      // In edge runtime or serverless, filesystem might not be available
    }
  }
