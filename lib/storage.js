...
import { getPublicRoot, getUploadDir, uploadPublicUrl } from '@/lib/uploadPaths';
...
export class StorageService {
  ...
  async uploadToFileSystem(buffer, filename, folder) {
    try {
      ...
      const uploadsDir = getUploadDir(folder);
      ...
      const hint =
        error.code === 'EACCES' || error.code === 'EROFS'
          ? ' Server cannot write to the uploads folder. On WHM/cPanel set folder permissions to 755 (or 775) for public/uploads, or set UPLOAD_DIR in .env to a writable path.'
          : '';
      throw new Error(`Failed to save file to filesystem: ${error.message}.${hint}`);
    }
  }
